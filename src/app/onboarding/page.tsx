...
const OnboardingPage = () => {
  ...
  return (
    <div className="flex flex-col items-center   w-full mx-auto max-w-7xl h-full px-4 lg:px-8">
      {/* steps */}
      <div className="flex items-center mb-4 justify-between md:justify-evenly w-full md:max-w-4xl md:px-4 mx-auto gap-x-4 relative">
        {STEPS.map((step) => (
          ...
          >
            <div
              className={cn(
                "w-10 h-10 flex font-bold items-center justify-center rounded-full text-sm",
                activeStep === step.id
                  ? "bg-primary dark:bg-slate-800 dark:text-slate-200 text-white"
                  : "dark:bg-slate-300 dark:text-black bg-zinc-100",
